...
public class TensorFlowClassifier implements Classifier {
    ...
    private native int[] classifyImageBmp(Bitmap bitmap);

    private native int[] classifyImageRgb(int[] output, int width, int height);
    ...
    public List<Recognition> recognizeImage(final Bitmap bitmap) {
        ...
        int[] buff = classifyImageBmp(bitmap);
        android.util.Log.d("TEST", ">>>>>>> buff length" + buff.length);
        Bitmap result = Bitmap.createBitmap(buff, bitmap.getWidth(), bitmap.getHeight(), Bitmap.Config.ARGB_8888);
    ...
    public Bitmap styleTransfer(final Bitmap bitmap) {
        int[] buff = classifyImageBmp(bitmap);
        return Bitmap.createBitmap(buff, bitmap.getWidth(), bitmap.getHeight(), Bitmap.Config.ARGB_8888);
    }
